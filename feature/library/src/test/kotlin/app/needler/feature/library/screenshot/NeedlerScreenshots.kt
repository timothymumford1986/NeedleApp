 ...
 * ## Why this is a copy
 *
 * The original lives in `:app`'s test source set, whose own KDoc says: "when
 * the second module wants it, it moves to a shared test module and nothing else
 * changes." `:feature:library` is that second module — but a test source set is
 * not published, so there is no way to depend on `:app`'s, and this work is not
 * allowed to edit `:app`. The right fix is a `:core:testing` module holding one
 * copy; it is in the handover notes. Until then this file is a deliberate
 * duplicate, kept byte-compatible in behaviour so the two sets of images are
 * directly comparable: same output directory, same device sizes, same density,
 * same reduced-motion rule.
 *
 * It adds one thing `:app`'s copy does not have: [fontScale], so a screen can
 * be rendered at the 200% text size REQUIREMENTS.md requires it to survive.
 */
...
enum class NeedlerDevice(
    ...
    /** `design/html/02-Library.html`: 390x844. */
