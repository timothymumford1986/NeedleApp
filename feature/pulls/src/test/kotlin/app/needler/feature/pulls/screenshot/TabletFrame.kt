 ...
 * without them a tablet render of Pulls would spread two columns of
 * 56dp-artwork rows across the whole 1280dp width, and the resulting PNG would
 * not show the screen the app will actually present.
 *
 ...
 * `sidebarWidth`. What that leaves between them is the content pane the app will
 * give this screen: 1280 − 96 − 400 = 784dp.
 *
 * The rail's Pulls item carries a badge, as every screen in the pack draws it.
 * REQUIREMENTS.md calls that number "the reliable channel" for pull state, and
 * having it in the frame is what makes a render show whether the screen and the
 * badge could ever contradict one another.
 */
@Composable
internal fun TabletFrame(badgeCount: Int = 2, content: @Composable () -> Unit) {
        ...
        NeedlerNavigationRail(
            items = listOf(
                NeedlerNavItem(
                    ...
                    onClick = {},
                    icon = { tint ->
                        NeedlerStrokeIcon(pathData = PATH_LIBRARY, tint = tint, size = 24.dp)
                    },
                ),
                NeedlerNavItem(
                    label = "Search",
                ...
                NeedlerNavItem(
                    ...
                    onClick = {},
                    icon = { tint ->
                        NeedlerStrokeIcon(pathData = PATH_SETTINGS, tint = tint, size = 24.dp)
                    },
                ),
            ),
        )
        ...
        Column(
            ...
            Text(
                text = "Artwork, transport, output and the crate are built in :feature:player " +
                    ...
                    "content pane beside it is the width Pulls really gets.",
