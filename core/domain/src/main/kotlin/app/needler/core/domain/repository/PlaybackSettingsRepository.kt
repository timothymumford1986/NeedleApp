 ...
 * via `savePlayQueue` is out of v1 scope. The crate lives here rather than in its own repository
 * because it is persisted device state with the same lifetime and the same single writer - the player
 * session.
 */
public interface PlaybackSettingsRepository {
    ...
    /** The persisted crate. Survives process death and restarts. */
    public fun observeQueue(): Flow<PlayQueue>

    /** Persists the crate. Called by the player session, which owns the live queue. */
    public suspend fun saveQueue(queue: PlayQueue)

    public suspend fun clearQueue()

    public suspend fun enqueue(items: List<QueueItem>, position: Int? = null)
